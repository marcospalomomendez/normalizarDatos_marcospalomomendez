...
class Program
{
    static void Main()
    {
        ...
        // Partidos
        var partidos = (from r in registros.Select((r, i) => new { Registro = r, Index = i })
                        select new Partido
                        {
                            Id = r.Index + 1,
                            Equipo1Id = equipos.First(e => e.Nombre == r.Registro.Equipo1).Id,
                            Equipo2Id = equipos.First(e => e.Nombre == r.Registro.Equipo2).Id,
                            CompeticionId = competiciones.First(c => c.Nombre == r.Registro.Competicion).Id,
                            Resultado = r.Registro.Resultado,
